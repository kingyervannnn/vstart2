...
const LABEL = 'com.vstart.environment-bridge'
const projectRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..')
const entrypoint = join(projectRoot, 'environment-bridge/index.mjs')
...
<plist version="1.0"><dict>
  ...
  <key>EnvironmentVariables</key><dict>
    ...
    <key>VSTART_LIGHT_CLI_PATH</key><string>/Users/vbitzx/.local/bin/room-light</string>
...
async function isLoaded() { return (await run('/bin/launchctl', ['print', serviceTarget], { allowFailure: true })).code === 0 }
...
async function start({ restart = false } = {}) {
  try { await readFile(plistPath) } catch { throw new Error('Environment Bridge is not installed. Run: npm run environment:bridge:manage -- install') }
...
else process.stdout.write('Usage: node environment-bridge/launchd.mjs <install|start|restart|stop|status|print-plist|uninstall>\n')
