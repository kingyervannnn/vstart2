...
The browser API supports Inbox, Sent, Starred, and Trash searches, reading, Gmail favorites through the `STARRED` label, draft creation, replies, forwarding as a new draft, attachments, draft listing, sending, and moving a message to Gmail Trash. The bridge owns a shared stale-while-revalidate inbox cache: it warms configured inboxes when the LaunchAgent starts, refreshes them once per minute, and coalesces concurrent reads from multiple tabs or devices. Compose suggestions are derived in memory from recent Sent recipients and Inbox senders only when compose is used, then cached briefly; V Start does not create a separate browser or database address book. Sending and trashing each require a separate explicit confirmation before the bridge invokes the corresponding guarded `mailctl` command. Favorite toggles are explicit UI actions and use the narrow guarded `mailctl star` / `mailctl unstar` commands. Gmail exposes no server-side Outbox label through this API; unsent messages remain in Drafts. Token management, permanent deletion, and arbitrary CLI execution are not exposed.
