...
import { inspectIconQuality, resolveShortcutIcon } from '../server/icons.mjs'
...
try {
  const result = await client.query(`
    SELECT s.id, s.title, s.url, s.icon_override_url, s.icon_asset_id,
           a.mime_type AS icon_mime_type, a.content AS icon_content
  ...
  for (const row of result.rows) {
    if (row.icon_asset_id && refreshWeak) {
      const quality = await inspectIconQuality(row.icon_content, row.icon_mime_type)
      if (quality.vector || quality.quality >= 64) continue
    }
    const icon = await resolveShortcutIcon(client, row.url, row.icon_override_url, { title: row.title })
    if (!icon.iconAssetId) {
      unavailable += 1
      continue
    }
