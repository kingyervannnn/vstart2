...
import { ArrowLeft, CloudSun, Droplets, FileText, Forward, GitBranch, Lightbulb, ListMusic, ListPlus, Mail, Music2, NotebookPen, Paperclip, Pause, PenLine, Play, Plus, RefreshCw, Repeat2, Reply, Save, Search, Send, Shuffle, SkipBack, SkipForward, Star, Thermometer, Trash2, Umbrella, Volume2, VolumeX, Wind, X } from 'lucide-react'
    ...
    <div className="music-service-view">
      ...
      <div className={'music-browser-grid ' + (query || search.loading || search.results.length || search.error ? 'with-search' : '')}>
        {capabilities.search === true && (query || search.loading || search.results.length || search.error) && <section className="music-search-panel"><h3><Search /> Search results</h3>
          ...
          {!search.loading && <div className="music-search-results">{search.results.map((result) => <article key={result.videoId}><button type="button" className="music-search-result-main" onClick={() => playResult(result)} aria-label={`Play ${result.title} now`}><MusicArtwork src={result.imageUrl} /><span><strong>{result.title}</strong><small>{result.detail}</small></span></button><button type="button" className="music-search-result-queue" onClick={() => addResult(result, 'INSERT_AT_END')} title="Add to queue" aria-label={`Add ${result.title} to queue`}><ListPlus /></button></article>)}{query && !search.loading && !search.results.length && !search.error && <div className="service-state">Search to load matching songs.</div>}</div>}
        </section>}
...
export function ServiceRailView({ kind, initialMailAccount, musicSettings, onMusicSettingsPatch, notesSettings, onNotesSettingsPatch, missionGlanceSettings, weatherSettings, workspaces = [], activeWorkspaceId, openLinksInNewTab, onOpenInline, onClose }) {
  const [state, setState] = useState({ loading: !['music', 'mail', 'notes', 'environment', 'mission-glance'].includes(kind), error: '', data: null })
  ...
  useEffect(() => {
    if (kind === 'music' || kind === 'mail' || kind === 'notes' || kind === 'environment' || kind === 'mission-glance') {
