...
describe('Music service view', () => {
  it('consolidates source and search into the header and exposes supported controls', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Voice music search' })).toBeTruthy()
    expect(screen.queryByRole('button', { name: 'Refresh queue' })).toBeNull()
  ...
  it('does not render controls the source reports as unsupported', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Voice music search' })).toBeNull()
  })
})
